...
QByteArray& QuotedPrintable::decode(const QString &input)
{
    ...
    for (int i = 0; i < input.length(); ++i)
    {
        if (input.at(i).toLatin1()/*toAscii*/ == '=')
        {
            int index=++i;
            output->append((hexVal[input.at(index).toLatin1()/*toAscii()*/ - '0'] << 4) + hexVal[input.at(++i).toLatin1()/*toAscii()*/ - '0']);
        }
        else
        {
            output->append(input.at(i).toLatin1()/*toAscii()*/);
        }
    }
